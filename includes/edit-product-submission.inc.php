...
// validate inputs
$sku = validate_edited_sku($productsArray,$sku,$product_id);
...
switch ("bad input"){
	...
	default:
		...
		if($sku == "sku already exists"){
			...
			$dataInsert = "UPDATE ShoppingCartProducts SET sku='$sku',product_name='$product_name',description='$description',image_url='$image_url',active='$active',size='$size',color='$color',price='$price',shipping_charge='$shipping_charge',category='$category' WHERE product_id='$product_id'";
			mysql_query($dataInsert, $c) or die ("some problem adding the product");
			print "<div style='color: green; font-weight:bold;' class='centeralign'><br><br>Changes are saved.</div>";
